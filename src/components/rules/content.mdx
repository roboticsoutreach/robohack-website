...
1. There are 12 radioactive cubes.
1. At the end of the match, cubes are scored as follows:
    1. Cubes in a team's territory are worth **1** point for the rival team.
    1. Cubes in a team's higher scoring zone are worth an additional **2** points for the rival team.
1. The tape separating the higher scoring zone **is** included in the scoring zone.
1. The rival territory for a robot is on the opposite side of the arena to the robot's starting zone.
1. A radioactive cube is in a territory if, and only if, the vertical projection of the cube is fully within the territory, where the tape denoting the territory **is not** included in this area.
...
1. A team's territory is split into 2 zones - the higher and lower scoring zones.
...
1. Robots must not deliberately or negligently damage the arena or anything in it.
1. Robots must fit within a 600 mm × 600 mm × 500 mm cube at all times.
